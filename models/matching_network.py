...
class MatchingNet(nn.Module):
    ...
    def forward(self, text, gather_positive, gather_negative, label_repre):
        ...
        label_positive = torch.gather(
            label_repre,
            1,
            gather_positive.view(text.size(0), self.positive_sample_num, 1).expand(
                text.size(0), self.positive_sample_num, label_repre.size(-1)
            ),
        )

        label_negative = torch.gather(
            label_repre,
            1,
            gather_negative.view(text.size(0), self.negative_sample_num, 1).expand(
                text.size(0), self.negative_sample_num, label_repre.size(-1)
            ),
        )
