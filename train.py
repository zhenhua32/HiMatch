...
def set_optimizer(config: Configure, model):
    """
...
def train(config: Configure):
    ...
    if "bert" in config.model.type:
        ...
        optimizer_grouped_parameters = [
            ...
            # 对于 no_decay 的参数，不使用正则项 weight_decay
    ...
    if not os.path.isdir(model_checkpoint):
        ...
        if os.path.isfile(os.path.join(model_checkpoint, latest_model_file)):
            ...
            logger.info(
                "Previous Best Performance---- Micro-F1: {}%, Macro-F1: {}%".format(
